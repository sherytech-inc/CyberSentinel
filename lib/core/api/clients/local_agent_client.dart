...
class LocalAgentClient {
  ...
  // ── Packets ───────────────────────────────────────────────────────────────
  static Future<Map<String, dynamic>> getPackets({int page = 1, int pageSize = 50}) async {
    return await _get('/api/v1/packets?page=$page&page_size=$pageSize');
  }

  // ── Firewall Logs ─────────────────────────────────────────────────────────
  static Future<Map<String, dynamic>> fetchFirewallLogs({int page = 1, int pageSize = 50}) async {
    return await _get('/api/v1/operations/firewall?page=$page&page_size=$pageSize');
  }

  static Future<Map<String, dynamic>> fetchFirewallActions({int page = 1, int pageSize = 50}) async {
    return await _get('/api/v1/firewall-action/actions?page=$page&page_size=$pageSize');
  }

  static Future<Map<String, dynamic>> uploadFirewallLogs(String fileName, List<int> bytes) async {
    try {
      final response = await _sendAuthenticated((headers) async {
        final request = http.MultipartRequest('POST', Uri.parse('$_baseUrl/api/v1/operations/firewall/upload'));
  ...
  // ── Threats ───────────────────────────────────────────────────────────────
  static Future<Map<String, dynamic>> getThreats({int page = 1, int pageSize = 50}) async {
  ...
  static Future<Map<String, dynamic>> getHighThreats({int page = 1, int pageSize = 50}) async {
    return await _get('/api/v1/threats/high?page=$page&page_size=$pageSize');
  }
  static Future<Map<String, dynamic>> getCriticalThreats({int page = 1, int pageSize = 50}) async {
    return await _get('/api/v1/threats/critical?page=$page&page_size=$pageSize');
  }
  static Future<Map<String, dynamic>> getThreatHistory({int page = 1, int pageSize = 100}) async {
    return await _get('/api/v1/threats/history?page=$page&page_size=$pageSize');
  }

  // ── Firewall Actions ──────────────────────────────────────────────────────
  static Future<Map<String, dynamic>> blockIP(String ip, {String? reason}) async {
    return await _post('/api/v1/firewall/block', body: {'ip': ip, 'reason': reason});
  }
  ...
  static Future<Map<String, dynamic>> startCapture({String interfaceName = 'en0', String? bpfFilter}) async {
  ...
  static Future<Map<String, dynamic>> getResponseThreats({int page = 1, int pageSize = 50}) async {
    return await _get('/api/v1/response/threats?page=$page&page_size=$pageSize');
  }
  ...
  static Future<Map<String, dynamic>> addThreatNote(String alertId, String note, {String author = 'Analyst'}) async {
    return await _post('/api/v1/response/threats/$alertId/notes', body: {'note': note, 'author': author});
  }
  static Future<Map<String, dynamic>> resolveThreat(String alertId, {String? notes}) async {
    return await _post('/api/v1/response/threats/$alertId/resolve', body: notes != null ? {'notes': notes} : {});
  }
  static Future<Map<String, dynamic>> ignoreThreat(String alertId, {String? notes}) async {
    return await _post('/api/v1/response/threats/$alertId/ignore', body: notes != null ? {'notes': notes} : {});
  }
  static Future<Map<String, dynamic>> responseBlockIP(String ip, {String? reason}) async {
    return await _post('/api/v1/response/block', body: {'ip': ip, 'reason': reason});
  }
  static Future<Map<String, dynamic>> responseUnblockIP(String ip, {String? reason}) async {
    return await _post('/api/v1/response/unblock', body: {'ip': ip, 'reason': reason});
  }
  static Future<Map<String, dynamic>> getResponseHistory({int page = 1, int pageSize = 50}) async {
    return await _get('/api/v1/response/history?page=$page&page_size=$pageSize');
  }
  static Future<Map<String, dynamic>> getResponseAuditLog({int page = 1, int pageSize = 50}) async {
    return await _get('/api/v1/response/audit-log?page=$page&page_size=$pageSize');
  }

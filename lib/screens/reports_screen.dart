...
class _ReportsScreenState extends State<ReportsScreen> {
  ...
  Widget build(BuildContext context) {
    return Consumer<ReportsProvider>(
      builder: (context, provider, child) {
        ...
        final severityData =
            data['severity_distribution'] as List<dynamic>? ?? [];
        final topThreats = data['top_threat_types'] as List<dynamic>? ?? [];
        final attackers = data['top_attackers'] as List<dynamic>? ?? [];
        final responseAnalytics =
            Map<String, dynamic>.from(data['response_analytics'] ?? {});
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          ElevatedButton.icon(
            onPressed: provider.isSnapshotGenerating
                ? null
  ...
  Widget _buildBreakdownCharts(
      List<dynamic> severityData, List<dynamic> threatTypes) {
    return Row(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Expanded(
              ...
              children: [
                  ...
                  child: severityData.isEmpty
                      ...
                      : PieChart(
                          PieChartData(
                            ...
                            sections: severityData
                                .where(
                                    (e) => (e['count'] as num).toDouble() > 0)
                                .map((e) {
                              final sev =
                                  e['severity'].toString().toUpperCase();
                ...
                Wrap(
                  ...
                  children: severityData.map((e) {
                    ...
                    Color c = AppTheme.info;
                    if (sev.toUpperCase() == 'CRITICAL') {
                      c = AppTheme.error;
                    } else if (sev.toUpperCase() == 'HIGH') {
                      c = Colors.orange;
                    } else if (sev.toUpperCase() == 'MEDIUM') {
                      c = AppTheme.warning;
                    }
                    return Row(
              ...
              : SingleChildScrollView(
                  ...
                  child: DataTable(
                    ...
                    rows: attackers.map((e) {
                      final severity =
                          (e['highest_severity'] ?? e['severity'] ?? 'UNKNOWN')
                              .toString()
                              .toUpperCase();
                      ...
                      if (severity == 'CRITICAL') {
                        ...
                        sevColor = AppTheme.success;
                      }

                      return DataRow(cells: [
                        DataCell(Text(
                            e['source_ip']?.toString() ??
                                e['ip']?.toString() ??
                                'N/A',
                            style:
                                const TextStyle(color: AppTheme.textPrimary))),
                        DataCell(Text(e['country']?.toString() ?? 'Unknown',
                            style:
                                const TextStyle(color: AppTheme.textPrimary))),
                        DataCell(Text(e['count']?.toString() ?? '0',
                            style:
                                const TextStyle(color: AppTheme.textPrimary))),
                        DataCell(Text(severity,
                            style: TextStyle(
                                fontWeight: FontWeight.w600, color: sevColor))),
                      ]);
                    }).toList(),
                  ),
                ),
  ...
  Widget _buildResponseTimeline(Map<String, dynamic> responseAnalytics) {
    final events = [
      if ((responseAnalytics['blocks'] ?? 0) > 0)
        {
          'title': 'IPs Blocked',
          'desc':
              'System blocked ${responseAnalytics['blocks']} malicious IPs.',
      ...
      if ((responseAnalytics['unblocks'] ?? 0) > 0)
        {
          'title': 'IPs Unblocked',
          'desc': 'System unblocked ${responseAnalytics['unblocks']} IPs.',
          'icon': LucideIcons.shieldCheck,
          'color': AppTheme.success
        },
      if ((responseAnalytics['investigations'] ?? 0) > 0)
        {
          'title': 'Investigations Started',
          'desc':
              '${responseAnalytics['investigations']} alerts are currently being investigated.',
      ...
      if ((responseAnalytics['resolved'] ?? 0) > 0)
        {
          ...
          'desc':
              '${responseAnalytics['resolved']} alerts have been successfully resolved.',
          'icon': LucideIcons.shieldCheck,
          'color': AppTheme.success
        },
      if ((responseAnalytics['ignored'] ?? 0) > 0)
        {
          ...
          'desc':
              '${responseAnalytics['ignored']} alerts were flagged as false positives.',
    ...
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const Text('Response Timeline Summary',
              style: TextStyle(
                  fontSize: 18,
                  fontWeight: FontWeight.w600,
                  color: AppTheme.textPrimary)),
          const SizedBox(height: 16),
          events.isEmpty
              ? const Text("No recent response activities.",
                  style: TextStyle(color: AppTheme.textSecondary))
              : ListView.builder(
                  shrinkWrap: true,
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ListTile(
                      leading: Icon(ev['icon'] as IconData,
                          color: ev['color'] as Color),
                      title: Text(ev['title'] as String,
                          style: const TextStyle(color: AppTheme.textPrimary)),
                      subtitle: Text(ev['desc'] as String,
                          style:
      ...
      child: Column(
        ...
        children: [
          ...
          Wrap(
            ...
            runSpacing: 16,
            children: [
              _buildExportBtn('Download PDF', 'pdf', provider.timeRange,
                  LucideIcons.fileText),
              _buildExportBtn(
                  'Export JSON', 'json', provider.timeRange, LucideIcons.code),
              _buildExportBtn('CSV: Alerts', 'csv_alerts', provider.timeRange,
                  LucideIcons.table),
              _buildExportBtn('CSV: Actions', 'csv_actions', provider.timeRange,
                  LucideIcons.table),
            ],
          ),
        ],
  ...
  Widget _buildExportBtn(
      String label, String type, String timeRange, IconData icon) {
